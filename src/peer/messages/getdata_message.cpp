 ...
 */
CGetDataMessage::CGetDataMessage(uint32_t n_magic)
 ...
 */
CGetDataMessage::CGetDataMessage(const std::vector<CInventoryItem>& vec_items, uint32_t n_magic)
 ...
 */
std::string CGetDataMessage::GetType() const {
 ...
 */
std::vector<uint8_t> CGetDataMessage::SerializePayload() const {
 ...
 */
bool CGetDataMessage::DeserializePayload(const std::vector<uint8_t>& payload) {
    m_vec_items.clear();
    ...
    for (uint32_t i = 0; i < n_count; i++) {
        ...
        // Validation: Check hash size is correct
 ...
 */
std::unique_ptr<CPeerMessage> CGetDataMessage::Clone() const {
    return std::make_unique<CGetDataMessage>(m_vec_items, GetMagic());
}

/**
 * @brief Get all requested items
 */
const std::vector<CInventoryItem>& CGetDataMessage::GetItems() const {
    return m_vec_items;
}
 ...
 */
void CGetDataMessage::AddItem(ObjectType::Type type, const std::string& str_hash) {
 ...
 */
size_t CGetDataMessage::GetItemCount() const {
