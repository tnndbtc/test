...
set LIBS_EXIST=1
set REQUIRED_LIBS=libbwpeer.dll libbwlogger.dll libbwthreadname.dll libbwblockcore.dll libbwrest.dll libbwutils.dll libbwpathutil.dll libbwpeermsg.dll
...
if %LIBS_EXIST% equ 0 (
    ...
    if defined PARALLEL_JOBS (
        ...
        cmake --build . --target bwthreadname bwlogger bwpeer bwutils bwpathutil bwblockcore bwrest bwpeermsg -j %PARALLEL_JOBS%
    ) else (
        echo Building libraries ^(this may take a few minutes^)...
        cmake --build . --target bwthreadname bwlogger bwpeer bwutils bwpathutil bwblockcore bwrest bwpeermsg
    )
    if %ERRORLEVEL% neq 0 (
        echo.
...
if exist "test_all.exe" (
    ...
    copy "%BUILD_DIR%\libbwpathutil.dll" . >nul 2>&1
